...
function isDerivedChargeOrBenefitRow(row: EditorRow) {
  const category = String(row.categoria || "").toLowerCase();
  const tags = Array.isArray((row as Record<string, unknown>).trainingTags)
    ? ((row as Record<string, unknown>).trainingTags as string[])
    : [];
  const autoGeneratedSource = String(
    (row as Record<string, unknown>).autoGeneratedSource || ""
  );
