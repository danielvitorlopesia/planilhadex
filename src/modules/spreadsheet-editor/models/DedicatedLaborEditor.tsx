...
function getRowExtraValue(row: EditorRow, key: string): unknown {
  const candidate = row as unknown as Record<string, unknown>;
  return candidate[key];
}

function getRowTrainingTags(row: EditorRow): string[] {
  const value = getRowExtraValue(row, "trainingTags");
...
function isDerivedChargeOrBenefitRow(row: EditorRow) {
  const category = String(row.categoria || "").toLowerCase();
  const tags = getRowTrainingTags(row);
  const autoGeneratedSource = String(
    getRowExtraValue(row, "autoGeneratedSource") || ""
  );
  ...
  return (
    ...
    autoGeneratedSource.startsWith("labor_")
  );
}
...
function recalcSubtotal(row: EditorRow): EditorRow {
  const quantidade = safeNumber(row.quantidade, 0);
  const valorUnitario = safeNumber(row.valorUnitario, 0);
  ...
  return [
    {
      ...
      automatico: true,
      autoGeneratedSource: "labor_inss",
    ...
    },
    {
      ...
      origem: "motor de encargos",
      automatico: true,
      autoGeneratedSource: "labor_fgts",
      trainingTags: ["generated_labor_charge"],
    },
    {
      id: "derived_rat",
    ...
    },
    {
      ...
      origem: "motor de encargos",
      automatico: true,
      autoGeneratedSource: "labor_third_party",
      trainingTags: ["generated_labor_charge"],
    },
    {
      id: "derived_ferias",
      ...
      origem: "motor de encargos",
      automatico: true,
      autoGeneratedSource: "labor_vacation_provision",
      trainingTags: ["generated_labor_charge"],
    },
    {
      ...
      origem: "motor de encargos",
      automatico: true,
      autoGeneratedSource: "labor_thirteenth_provision",
      trainingTags: ["generated_labor_charge"],
    },
    {
      ...
      automatico: true,
      autoGeneratedSource: "labor_transport_allowance",
    ...
    },
    {
      ...
      origem: "motor de benefícios",
      automatico: true,
      autoGeneratedSource: "labor_meal_allowance",
      trainingTags: ["generated_labor_benefit"],
    },
    {
      ...
      origem: "motor de benefícios",
      automatico: true,
      autoGeneratedSource: "labor_other_benefits",
      trainingTags: ["generated_labor_benefit"],
    },
  ].filter((row) => Number(row.subtotal || 0) > 0) as EditorRow[];
}
...
export default function DedicatedLaborEditor({
  ...
  function handleSave() {
    try {
      const sanitizedConfig = sanitizeLaborChargesConfig(chargesConfig);
      ...
      const sanitizedLaborRows = rows.map((row) =>
        recalcSubtotal({
          ...
          quantidade: Math.max(0, safeNumber(row.quantidade, 0)),
          valorUnitario: Math.max(0, safeNumber(row.valorUnitario, 0)),
      ...
      const updated = updateSpreadsheet(
        spreadsheet.id,
        {
          ...
          metadata: {
            ...
            laborChargesConfig: sanitizedConfig,
      ...
      <CardContent>
        <Stack spacing={2.5}>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
            ...
            />
            <Chip
              label={`Total encargos: ${formatCurrency(laborCost.totalEncargos)}`}
              variant="outlined"
            />
            <Chip
              label={`Total benefícios: ${formatCurrency(laborCost.totalBenefits)}`}
              variant="outlined"
            />
          </Stack>
          ...
          <Box sx={{ overflowX: "auto" }}>
            <Table size="small">
              ...
              <TableBody>
                {rows.length > 0 ? (
                  rows.map((row, index) => (
                    <TableRow key={String(row.id || `${String(row.item)}-${index}`)}>
                      <TableCell>
                        <EditableCell
